...
### Increment 2 RESULT (2026-06-02, measured) — parallelism: correct but MODEST, NOT the hoped 2–3×
Shipped Tier 2: async session path + bounded `AsyncSessionPool` (cap `MAX_CONCURRENT=3`) + concurrent `asyncio.gather` researcher fan-out + rate-limit-safe permit release. **A live-only bug shipped through CI** (the parallel fan-out called the SYNC critique brains inside the running `asyncio.run` loop → nested-loop conflict → `coroutine connect never awaited` → **0 evidence**); the deterministic tests mocked the brains so missed it. **Fixed** (Option A: critique brains run sequentially AFTER `asyncio.gather` returns, outside the async loop) + added a regression test (`_SyncLoopBrain` doing `asyncio.run` inside the fan-out — fails on old code, passes on fix). 122 tests green.
- **Same benchmark, after fix: loop latency 1072s → 795.9s (~26% faster loop); `cost=$1.48, calls=15`; evidence 7 / 48 sources (correct again).** BUT total run wall-clock ~17 min — **roughly unchanged**.
- **Honest verdict: parallelism gave only ~26% on the loop, NOT 2–3×.** Most likely the **subscription rate limit serializes concurrent `claude` spawns** (a `five_hour` `RateLimitEvent` was observed in diag), so 3 "parallel" WebSearch calls don't truly overlap; plus cap=3 with only ~3 researchers/iter limits the parallel surface. **The real win was increment 1** (cap-fan-out cut work 92→48 sources + persistent session). Increment 2 is correct + a net-positive (modest loop speedup + the parallel architecture for when rate limits lift / for non-subscription API keys) but NOT the breakthrough.
- **Revised P2 outlook:** under subscription rate limits, ~13–17 min / 2-iter appears to be near the floor for this many WebSearch calls. Bigger wins now come from **doing FEWER calls** (Tier 4 deeper: tighter top-K, dedup, skip-low-impact) + **prompt caching** (cost) + possibly an **API-key path** (no subscription rate-limit) for batch/overnight runs. True parallelism pays off there.
