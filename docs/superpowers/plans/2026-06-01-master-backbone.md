...
**Borrowed from `skill-ai-escape-mrc` (user-approved 2026-06-01, deep-dive 2026-06-01):**
- (7) **Exhaustion, not "done" — never fake-finish.** Mirrors escape-mrc's `EXHAUSTED/REWORK/CONTINUE` verdict + ADDRESSABLE-vs-RESIDUAL weakness classification. Every open issue/challenge must land in exactly one bucket: **answered** OR **can't-answer-and-why** (RESIDUAL = blocked_by_human / internal_data / permission / decision). `terminal_stop` is allowed ONLY when the "still self-answerable" bucket is empty (no ADDRESSABLE items left), never on a score threshold. This replaces pass/fail readiness with an exhaustion model so no weakness hides behind a number. Implements §4.1/§4.3 + §9.4 + §10. Deterministic → **P1** (COS decision + stop logic). Source pattern: `phase_3_rc_audit.py`, `schemas.py:RC_AUDIT`, `graph.py:_route_after_rc_audit` (+ env-capped attempt counter to bound the rework loop).
- (8) **No audit, no memo — emission gate.** Mirrors escape-mrc `phase_7:_assert_emission_prerequisites`. The `synthesis_node` REFUSES to produce the final memo if the Albert audit (or counterargument/readiness checks) did not genuinely run — i.e. ran on a **mock**, **errored**, or was **skipped** (a degraded/fallback audit is NEVER allowed to count as "passed" or to drive `terminal_stop`). Bypass only via an explicit recorded reason (env flag + reason), never silently. This is the structural enforcement of §10 anti-premature-end and the guard for consuming the in-progress external `skill-cn5-i-am-albert`. Deterministic gate → **P1**; the degraded-audit guard matters most in **P2** when the real/external Albert can fail. Also kills the wiki R13 "degraded-emission-with-warning" anti-pattern at the output boundary.
...
- **C3 Document handling conventions:** `reference/pdf/` (originals) + `reference/fragments/` (docling/pymupdf4llm-converted MD; raw pdftotext banned — loses tables) + a `reference-map` registry; **structural provenance YAML is the citation source-of-truth, NOT embeddings/RAG.** → **P4** doc adapter.

  **TL;DR (user-confirmed 2026-06-01):** the deciding factor is CONSUMER + PURPOSE, not document size — **human/Claude reading to UNDERSTAND → read the PDF directly (never convert); a PROGRAM that must machine-verify citations / extract structured tables / persist a reusable cited fragment → convert (only the target section).** For our near-term work (Claude helping read docs) we essentially never convert; docling conversion is P4 worker machinery.

  **PDF-handling decision table (user-confirmed 2026-06-01):**

  | 情境 | 轉 MD? | 怎麼做 |
  |---|---|---|
  | P1 (zero-LLM, no real docs) | No | — |
  | Claude reads a few docs ad-hoc in-session | No | Read tool reads PDF directly (`pages` range param) |
  | P4 programmatic worker needs machine-checkable citations / table fidelity | **Yes** | docling → `reference/fragments/`, cached once |

  **Cost model (token vs compute — user concern 2026-06-01):** docling PDF→MD conversion is a **LOCAL parsing library** (CPU/GPU, seconds–minutes) → it costs **local compute, ZERO LLM tokens**. LLM tokens are spent ONLY when text is loaded into a model's context, and are **proportional to the amount of text loaded** — identical whether you read the PDF directly or read converted MD, *as long as you only load the target section*. Nuance: the Read tool renders PDF pages **visually** (image tokens), so direct PDF read can cost MORE tokens/page than plain-text MD; converting to MD lowers per-use token cost but pays a one-time local-compute conversion. So: **one-off / read-once → read target pages directly (skip conversion compute); reused-many-times or needs citations/table-fidelity → convert target section once, cache, amortize.**

  **Large-PDF rule (NEVER ingest the whole file):** for a big PDF you only need part of, do selective extraction — **(1) outline/TOC scan** (cheap; PDF bookmarks or a `pdftotext` grep — discovery only, NOT conversion) to locate the section's page range → **(2) page-range extract** that subset only → **(3) docling-convert ONLY that fragment** (if conversion is warranted per cost model). Never read/convert all N pages: a 9000-page PDF read whole ≈ 9000 pages of tokens + blows context. Both Claude (Read `pages` param) and the worker (outline→page-range→convert, the reusable paperwork `pdf_outline_scan.py`→`extract_page_range_pdf.py`→`convert-fragment` front-half) follow locate-then-touch-only-the-target — this is the single lever that saves BOTH tokens and compute.
